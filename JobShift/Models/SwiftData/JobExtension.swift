...
extension Job {
    ...
    func getPaymentDay(year: Int, month: Int) -> Date {
        ...
        var paymentDay = Date(year: year, month: month, day: min(self.salaryPaymentDay, daysInMonth))
    ...
    func getWorkInterval(year: Int, month: Int?) -> DateInterval {
        let startMonth = ((month ?? 1) - (self.salaryPaymentType == .nextMonth ? 3 : 2) % 12) + 1
        ...
        let startDate = Date(year: year, month: startMonth, day: min(self.salaryCutOffDay + 1, startDaysCount))
        let endDate = Date(year: year, month: endMonth, day: min(self.salaryCutOffDay, endDaysCount))
