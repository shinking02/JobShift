...
final class SalaryManager {
    ...
    func getSalaries(jobs: [Job], otJobs: [OneTimeJob], year: Int, month: Int?) -> [Salary] {
        ...
        salaries = jobs.map { job in
            ...
            var forcastWage = 0
            events.forEach { e in
                forcastWage += calcSalaryByEvent(job: job, event: e.gEvent)
            }
            let commuteWage = job.isCommuteWage ? job.commuteWage * events.count : 0
            var isConfirmed = false
            let confirmedWageIncludedCommute = {
            ...
            return Salary(
                        ...
                        confirmedWageIncludedCommute: confirmedWageIncludedCommute,
        ...
        salaries.append(Salary(
            isConfirmed: true, confirmedWageIncludedCommute: otConfirmed, forcastWage: otConfirmed, commuteWage: otCommute, events: []
        ))
        return salaries
    }
    
    func calcSalaryByEvent(job: Job, event: GTLRCalendar_Event) -> Int {
    ...
    func calculateDates(year: Int, month: Int?, day: Int) -> (salaryStart: Date, salaryEnd: Date) {
        ...
        if let month = month {
            ...
            let endComponents = {
                ...
                if endMonthDays < day {
                    return DateComponents(year: year, month: month + 1, day: 0)
                } else {
...
struct Salary {
    ...
    var confirmedWageIncludedCommute: Int
    var forcastWage: Int
    var commuteWage: Int
